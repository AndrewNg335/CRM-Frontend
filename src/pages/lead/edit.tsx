...
export const LeadDetailPage = () => {
  ...
  // Owner select for Admin (opportunity modal)
  ...
  const {
    formProps: opportunityFormProps,
    modalProps: opportunityModalProps,
  ...
  const {
    formProps: interactionFormProps,
    modalProps: interactionModalProps,
  ...
  const {
    formProps: editReminderFormProps,
    modalProps: editReminderModalProps,
  ...
  // Auto-set ownerId and leadId for opportunity form
  useEffect(() => {
  ...
  // Reset and set form values when opportunity modal is opened
  useEffect(() => {
  ...
  // Auto-set userId for interaction form
  useEffect(() => {
  ...
  // Reset and set form values when interaction modal is opened
  useEffect(() => {
  ...
  // Auto-set userId and leadId for reminder form
  useEffect(() => {
  ...
  // Reset and set form values when reminder modal is opened
  useEffect(() => {
  ...
  if (!lead) return <div>No data</div>;
  ...
  return (
    <div className="page-container">
          ...
          <Card bordered={false} className="professional-card" style={{ marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <Space>
                <EditButton
                  size="small"
                  recordItemId={lead._id}
                  onClick={() => show(lead._id)}
            ...
            <Row gutter={[24, 16]}>
              <Col span={12}>
                ...
                <DetailRow label="Trạng thái" value={lead.status} />
                <DetailRow label="Số điện thoại" value={lead.phone} />
                <DetailRow
                  label="Email"
                  value={<a href={`mailto:${lead.email}`} style={{ color: '#1890ff' }}>{lead.email}</a>}
                />
              ...
              <Col span={12}>
                ...
                <DetailRow
                  ...
                  value={dayjs(lead.lastInteractionDate).format(
                    "DD/MM/YYYY HH:mm"
                  )}
                />
                    ...
                    <div>
                      ...
                      {interactionsLoading ? (
                        ...
                        <Empty 
                          ...
                          description="Chưa có tương tác nào"
                          style={{ margin: '20px 0' }}
                        />
                      ) : (
                        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                          {interactions.map((interaction: any) => (
                            <InteractionCard
                              ...
                              onDelete={(id) => {
                                // Optimistic update list, then refetch list and lead summary
                  ...
                  children: (
                    <div>
                      ...
                      {opportunitiesLoading ? (
                        ...
                        <Empty 
                          image={Empty.PRESENTED_IMAGE_SIMPLE}
                          description="Chưa có cơ hội nào"
                          style={{ margin: '20px 0' }}
                        />
                      ) : (
                        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
      ...
      {/* Modal thêm interaction */}
      <InteractionModal
        modalProps={interactionModalProps}
        formProps={interactionFormProps}
        initialValues={{ leadId: lead._id }}
      />
      ...
      <ReminderCreateModal
        modalProps={reminderModalProps}
        formProps={reminderFormProps}
        leadSelectProps={{ options: [] }} // Empty options since leadId is set via initialValues
        initialValues={{ leadId: lead._id }}
        hideLeadSelect={true} // Hide lead select since we're in lead detail page
      />
      ...
      {/* Modal thêm opportunity */}
      <OpportunityCreateModal
        modalProps={opportunityModalProps}
        formProps={opportunityFormProps}
        ownerSelectProps={ownerSelectProps}
        leadSelectProps={{ options: [] }} // Empty options since leadId is set via useEffect
        campaignSelectProps={{ options: [] }} // Empty options for now
        isAdmin={isAdmin}
        initialValues={{ leadId: lead._id }}
        hideLeadSelect={true} // Hide lead select since we're in lead detail page
      />
