...
export const LeadList = ({ children }: React.PropsWithChildren) => {
  ...
  // Auto-set responsibleUserId for edit form
  React.useEffect(() => {
  ...
  // Owner select for Admin (create modal)
  ...
  // Auto-set responsibleUserId and default status for create form
  React.useEffect(() => {
    if (createFormProps?.form && currentUser?._id) {
      createFormProps.form.setFieldsValue({
        responsibleUserId: currentUser._id,
        status: 'new', // Set default status to "Mới"
      });
    }
    ...
    <div className="page-container">
      ...
      <Row gutter={[16, 16]} className="mb-6">
        ...
        <Col xs={24} sm={6}>
          <FilterableStatsCard
            title="Đang liên hệ"
            value={getStatsData().contacting}
            color="#f59e0b"
            isActive={activeFilter === 'contacting'}
            onClick={() => handleFilterClick('contacting')}
          />
        </Col>
        <Col xs={24} sm={6}>
          <FilterableStatsCard
            title="Khách hàng mới"
            value={getStatsData().new}
            color="#8b5cf6"
      ...
      <Card className="shadow-lg border-0 rounded-lg overflow-hidden">
        ...
        >
          ...
          <Table.Column<Lead>
            ...
            defaultFilteredValue={getDefaultFilter('search', filters)}
            filterIcon={<SearchOutlined />}
            filterDropdown={(props) => (
              <FilterDropdown {...props}>
          ...
          {/* Responsible User column - only visible for Admin */}
          {isAdmin && (
            <Table.Column<Lead>
              ...
              render={(responsibleUserId, record) => {
                // Find the responsible user from the users list
                const responsibleUser = ownerSelectProps?.options?.find(
                  (user: any) => user.value === responsibleUserId
                );
                return (
                  <div className="flex items-center space-x-2">
              ...
              <Space size="small">
                <Tooltip title="Xem chi tiết">
                  <Button 
                    icon={<EyeOutlined />} 
                    size="small" 
                    onClick={() => go({ to: `/leads/edit/${value}` })}
