...
export const UserList = ({ children }: React.PropsWithChildren) => {
  ...
  const { tableProps, filters, searchFormProps } = useTable<User, HttpError, User>({
    resource: "auth",
    meta: {
      customUrl: "http://localhost:3000/auth",
    },
  ...
  const rowSelection = {
    ...
    getCheckboxProps: (record: User) => ({
      disabled: record._id === currentUser?._id, // Disable checkbox for current user
      name: record.name,
    }),
  };
  ...
  const {
    ...
    resource: "auth",
    meta: {
      customUrl: (id: string) => `http://localhost:3000/auth/adminUpdate/${id}`,
    },
    redirect: false,
  ...
  const { data: rolesData } = useList({
    ...
    pagination: { mode: "off" },
    queryOptions: {
      enabled: isAdmin, // Only fetch roles if user is Admin
    },
  });
  ...
  // Check permissions: Only Admin can access user management pages
  if (!isAdmin) {
    return (
    ...
    <div className="page-container">
      ...
      <Card className="shadow-lg border-0 rounded-lg overflow-hidden">
        ...
        >
          ...
          <Table.Column<User>
          dataIndex="taskAssignedCount"
            title={
              <div className="flex items-center space-x-2">
                <TeamOutlined className="text-cyan-500" />
                <span className="font-semibold">Số task</span>
              </div>
            }
            render={(count) => (
              <Typography.Text className="text-gray-700">
            ...
            width={100}
            sorter={(a, b) => (a.taskAssignedCount || 0) - (b.taskAssignedCount || 0)}
          />

          <Table.Column<User>
          dataIndex="leadAssignedCount"
            title={
              <div className="flex items-center space-x-2">
                <UserOutlined className="text-indigo-500" />
                <span className="font-semibold">Số lead</span>
              </div>
            }
            render={(count) => (
              <Typography.Text className="text-gray-700">
            ...
            width={100}
            sorter={(a, b) => (a.leadAssignedCount || 0) - (b.leadAssignedCount || 0)}
          />
              ...
              <Space size="small">
                <Tooltip title="Xem chi tiết">
                  <Button 
                    icon={<EyeOutlined />} 
                    size="small" 
                    onClick={() => go({ to: `/users/show/${value}` })}
