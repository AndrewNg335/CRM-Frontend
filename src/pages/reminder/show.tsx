...
export const ReminderShow = () => {
    ...
    const { data: userData } = useOne({
        resource: "auth",
        id: reminder?.userId,
        meta: { customUrl: reminder?.userId ? `${API_URL}/auth/${reminder.userId}` : undefined },
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {isOverdue && (<Tag color="red" className="px-3 py-1 rounded-full font-medium"><ClockCircleOutlined className="mr-1"/>Quá hạn</Tag>)}
              </div>
            </div>
          </div>
          <Space>
            <Button type="primary" icon={<EditOutlined />} onClick={() => showEditModal(reminder._id)} className="professional-button" size="large">
              Chỉnh sửa
            </Button>
          </Space>
        </div>
        ...
        <Col xs={24} lg={16}>
          ...
          <Card className="professional-card" title={<div className="flex items-center space-x-2">
            ...
            <Timeline>
              ...
              {reminder.updatedAt && (<Timeline.Item dot={<CalendarOutlined style={{ color: "#8c8c8c" }}/>} color="#8c8c8c">
