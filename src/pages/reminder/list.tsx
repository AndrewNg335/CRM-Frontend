...
export const ReminderList = ({ children }: React.PropsWithChildren) => {
  ...
  // Auto-set userId when edit form is initialized
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if (queryResult?.data?.data && editFormProps?.form) {
      ...
      console.log('Reminder edit data loaded:', reminderData); // Debug log
      editFormProps.form.setFieldsValue({
  ...
  // Auto-set userId and ownerId when create form is initialized
  React.useEffect(() => {
    if (createFormProps?.form && currentUser?._id) {
      createFormProps.form.setFieldsValue({
        userId: currentUser._id,
        ownerId: currentUser._id, // Default to current user as owner
      });
    }
  }, [createFormProps?.form, currentUser?._id]);

  // Reset form values when create modal is opened
  React.useEffect(() => {
    if (createModalProps?.open && createFormProps?.form && currentUser?._id) {
      ...
      createFormProps.form.setFieldsValue({
        ...
        reminderStatus: 'PENDING', // Set default status
        priority: 'MEDIUM', // Set default priority
        repeat: 'NEVER', // Set default repeat
      });
    }
  ...
  // Owner select for Admin (reminder modals)
  ...
  const { tableProps, filters, searchFormProps } = useTable<Reminder, HttpError, { q?: string }>({
        ...
        <Card className="mb-4" bodyStyle={{ padding: '16px 24px' }}>
          <Form {...searchFormProps} layout="inline" style={{ width: '100%' }}>
            <Form.Item name="q" style={{ marginBottom: 0, width: '100%' }}>
              <Input.Search
                placeholder="Tìm kiếm nhắc nhở theo tiêu đề hoặc nội dung nhắc nhở"
                size="large"
      ...
      <Card className="shadow-lg border-0 rounded-lg overflow-hidden">
        ...
        >
          ...
          <Table.Column<Reminder>
            ...
            render={(timeReminder, record) => {
              ...
              return (
                <div className="flex flex-col space-y-1">
                  <Typography.Text 
                    className={`text-gray-700 ${isOverdue ? 'text-red-600 font-semibold' : ''}`}
                  >
            ...
            width={200}
            sorter={(a, b) => {
              if (!a.timeReminder) return 1;
              if (!b.timeReminder) return -1;
          ...
          <Table.Column<Reminder>
            ...
            render={(status) => (
              <div className="flex items-center gap-1">
                <Badge
                  status={status === 'DONE' ? "success" : status === 'PENDING' ? "processing" : "error"}
          ...
          <Table.Column<Reminder>
            ...
            width={120}
            sorter={(a, b) => {
              if (!(a as any).createdAt) return 1;
              if (!(b as any).createdAt) return -1;
              ...
              <Space size="small">
                <Tooltip title="Xem chi tiết">
                  <Button
                    icon={<EyeOutlined />}
                    size="small"
                    onClick={() => go({ to: `/reminders/show/${value}` })}
                ...
                <Tooltip title="Chỉnh sửa">
                  <Button
                    ...
                    size="small"
                    onClick={() => {
                      console.log('Edit reminder clicked:', value); // Debug log
                      showEditModal(value);
                    }}
