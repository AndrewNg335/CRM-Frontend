...
const List = ({ children }: React.PropsWithChildren) => {
    ...
    const { data: tasks, isLoading: isLoadingTasks, refetch } = useList<Task>({
        resource: isAdmin ? "tasks" : `tasks/user/${currentUser?._id}`,
    ...
    return (<>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button onClick={() => go({ to: "/tasks/new", type: "replace" })} className="professional-button" style={{
            padding: "8px 16px",
            background: "#3b82f6",
            color: "#fff",
            border: "none",
      ...
      <div>
        ...
        <KanbanBoard onDragEnd={handleOnDragEnd}>
          {taskStages.map((column) => (<KanbanColumn key={column.id} id={column.id} title={column.title} count={column.tasks.length}>
              {isLoading && <ProjectCardSkeleton />}
              {!isLoading &&
                column.tasks.map((task) => (<KanbanItem key={task._id} id={task._id} data={{ ...task }}>
...
const PageSkeleton = () => {
    ...
    return (<KanbanBoardContainer>
      {Array.from({ length: columnCount }).map((_, index) => (<KanbanColumnSkeleton key={index}>
          {Array.from({ length: itemCount }).map((_, idx) => (<ProjectCardSkeleton key={idx}/>))}
