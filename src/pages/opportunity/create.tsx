// src/pages/opportunity/create.tsx
...
export const CreateOpportunity = () => {
  ...
  // Auto-set ownerId when form initializes
  React.useEffect(() => {
  ...
  const ownerSelectProps = React.useMemo(() => {
    ...
    // Exclude Admin users from options
    const filtered = options.filter((opt: any) => {
      // rawOwnerSelectProps does not include role data by default; fallback to label text check or keep as-is
      // If server includes role in label, we could parse; otherwise keep all and rely on backend validation.
      return true;
    });
          ...
          <Row gutter={[24, 16]}>
            ...
            <Col xs={24} md={12}>
              <ProfessionalFormItem
                label="Chiến dịch"
                name="campaignId"
                icon={<LinkOutlined className="text-purple-500" />}
              >
                <Select {...campaignSelectProps} placeholder="Chọn chiến dịch" />
              </ProfessionalFormItem>
            </Col>
          </Row>
