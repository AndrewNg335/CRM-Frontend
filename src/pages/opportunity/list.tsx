...
export const OpportunityList = ({ children }: React.PropsWithChildren) => {
  ...
  const getStageIndex = (stage: string) =>
    stageList.findIndex((s) => s.value === stage);

  const getStepStatus = (i: number, currentStage?: string) => {
    if (!currentStage) return "wait";
    ...
    if (currentStage === "CLOSED_LOST") {
      if (i < currentIndex) return "wait";
      if (i === currentIndex) return "error";
      return "wait";
    }
  ...
  const { formProps: editFormProps, modalProps: editModalProps, show: showEditModal } = useModalForm<Opportunity>({
    ...
    onMutationSuccess: () => {
      // Invalidate both admin and user-specific resources
      invalidate({
        resource: "opportunities",
  ...
  // Auto-set ownerId for edit form
  React.useEffect(() => {
  ...
  // Owner select for Admin (edit modal)
  ...
  const formatCurrency = (amount: number) => {
    try {
      ...
      // Check if number is too large for safe integer
        ...
        deleteOpportunity(
          ...
          },
          {
            onSuccess: () => {
              ...
              // Invalidate data to refresh the list
              invalidate({
                resource: "opportunities",
                invalidates: ["list", "many"],
              });
  ...
  return (
    <div className="page-container">
        ...
        <Card className="mb-4" bodyStyle={{ padding: '16px 24px' }}>
          <Form {...searchFormProps} layout="inline" style={{ width: '100%' }}>
            <Form.Item name="name" style={{ marginBottom: 0, width: '100%' }}>
              <Input.Search
                placeholder="Tìm kiếm cơ hội theo tên hoặc khách hàng..."
                size="large"
      ...
      <Card className="shadow-lg border-0 rounded-lg overflow-hidden">
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) => 
              `${range[0]}-${range[1]} của ${total} cơ hội`,
        ...
        >
          ...
          <Table.Column<Opportunity>
            ...
            render={(nextStep) => (
              <Typography.Text className="text-gray-700" ellipsis>
                {nextStep || "-"}
              </Typography.Text>
            )}
            width={180}
              ...
              <Space size="small">
                ...
                <Tooltip title="Xóa">
                  <Button
                    icon={<DeleteOutlined />}
                    size="small"
                    onClick={() => handleDeleteOpportunity(record._id)}
              ...
              <Card className="mb-2" title="Giai đoạn" bordered>
                ...
                >
                  {stageList.map((stage, i) => (
                    <Step
                      key={stage.value}
                      title={opportunityStageLabel[stage.value]}
                      status={getStepStatus(i, selectedOpportunity.opportunityStage)}
                    />
                  ))}
                </Steps>
              </Card>
              ...
              <Card title="Chi tiết" bordered>
                <Row gutter={[16, 16]}>
                  <Col span={12}>
                    ...
                    <DetailRow
                      label="Giai đoạn"
                      value={opportunityStageLabel[selectedOpportunity.opportunityStage]}
                    />
                  </Col>
                  <Col span={12}>
                    <DetailRow label="Lead"
                      value={typeof selectedOpportunity.leadId === "string"
                        ? selectedOpportunity.leadId
                        : selectedOpportunity.leadId?.name}
                    />
                    <DetailRow label="Chiến dịch"
                      value={typeof selectedOpportunity.campaignId === "string"
                        ? selectedOpportunity.campaignId
                        : selectedOpportunity.campaignId?.name}
                    />
                    ...
                    <DetailRow label="Nguồn khách hàng" value={selectedOpportunity.leadSource} />
      ...
      <OpportunityEditModal
        ...
        campaignSelectProps={campaignSelectProps}
        isAdmin={isAdmin}
      />
