...
export const RoleInfoCard: React.FC<RoleInfoCardProps> = ({ role, compact = false }) => {
    ...
    if (compact) {
        ...
        <Tag color={getRoleColor(role.name)} className="px-2 py-1 rounded-full font-medium text-xs">
    ...
    return (<Card size="small" className="border border-gray-200">
      <div className="space-y-3">
        ...
        {role.permissions && role.permissions.length > 0 && (<div>
            ...
            <PermissionDisplay permissions={role.permissions} maxDisplay={5} showCategories={false}/>
          </div>)}
      </div>
    </Card>);
};