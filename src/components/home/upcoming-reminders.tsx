...
export const UpcomingReminders = () => {
    ...
    return (<Card className="professional-card relative overflow-hidden" style={{ height: "100%" }} styles={{
      ...
      {isLoading ? (<div style={{ padding: "16px 20px" }}>
          <List itemLayout="horizontal" dataSource={Array.from({ length: 5 }).map((_, index) => ({
                id: index,
            }))} renderItem={() => <UpcomingEventsSkeleton />}/>
        </div>) : reminders.length === 0 ? (<div style={{ padding: "40px 20px" }}>
          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<AntText style={{ color: "#6b7280", fontSize: 14 }}>
          ...
          <List itemLayout="horizontal" dataSource={reminders} split={false} renderItem={(item, index) => {
                const renderDate = () => item.endDate
                ...
                return (<List.Item style={{
                        ...
                        cursor: "pointer"
                    }} className="reminder-list-item" onClick={() => push(`/reminders/show/${item.id}`)}>
                  <div style={{ display: "flex", alignItems: "flex-start", width: "100%" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                        ...
                        <Tag color={item.status.color} style={{
                        fontSize: 11,
                        borderRadius: 6,
                        margin: 0,
                        display: "flex",
